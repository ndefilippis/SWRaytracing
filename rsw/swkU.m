function [Sout,time,ke,pe,Xp,hmov] = swkU(Sin,Psi,f,Cg,numsteps,savestep,Xpin)
...
Km2_ = K_.^(-2);
Km2_(kmax+1,1) = 0;  % set (0,0) NaN to 0
...
while keepgoing
    ...
    if (killpv)
        % reset vorticity = f*h, but keep divergence:
        Sk(:,:,1) = -Km2_.*(ikx_.*divuk - f*iky_.*Sk(:,:,3));
        Sk(:,:,2) = -Km2_.*(iky_.*divuk + f*ikx_.*Sk(:,:,3));
    end
...
%  u_t = -U*u_x - V*u_y - u*U_x - v*U_y + f*v - Cg^2*h_x + nu*del^(a) u
%      = -2*(U*u)_x - (V*u + v*U)_y + U*divu + f*v - Cg^2*h_x + nu*del^(a) u
%  v_t = -U*v_x - V*v_y - u*V_x - v*V_y - f*u - Cg^2*h_y + nu*del^(a) v
%      = -(U*v + u*V)_x - 2*(V*v)_y + V*divu - f*u - Cg^2*h_y + nu*del^(a) v
%  h_t = -(U*h)_x -(V*h)_y -(u_x + v_y) 
...
function [hc] = plotstuff(frame,xp,yp)
    ...
    pcolor(x,x,real(zeta)'), shading interp, colorbar, axis image
    clim = max([abs(min(real(zeta(:)))) abs(max(real(zeta(:))))]);
    caxis([-clim clim])
