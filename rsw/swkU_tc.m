function [Sout,time,ke,pe,Xp,hmov] = swkU_tc(Sin,Psi,f,Cg,numsteps,savestep, x_, y_, km, acs, ag, raXT, Xpin)
...
while keepgoing
    ...
    % Adapt dt and nu
    dt = 4e-4;%dttune*dx/Umax;   % Courant condition 
    nu = nudt/dt;
...
%     Psi_t = Cg^2/f*( sin(km*x_).*sin(km*y_)+a*cos(km*x_).*cos(km*y_) );
%     Psi_t = ag*Psi_t/max(max(abs(Psi_t(:,:,1))));
    Psi_t = CS_streamfunction(Cg, f, km, acs, ag, t, x_, y_, raXT);
    Psik = g2k(Psi_t);
    U = -k2g(iky_.*Psik);
    V = k2g(ikx_.*Psik);
...
function Psi_cs = CS_streamfunction(Cg, f, km, acs, ag, t, x_, y_, raXT)
    Psi_cs = Cg^2/f*( sin(km*(x_-t*raXT)).*sin(km*(y_-t*raXT)) + acs*cos(km*(x_-t*raXT)).*cos(km*(y_-t*raXT)) );
    Psi_cs = ag*Psi_cs/max(max(abs(Psi_cs(:,:,1))));
    return
...
function Sk_new = PV_damping(Sk)
   global ikx_ iky_ 
   f = 3;
   Cg = 1;
   alpha = 1;
   vxMuy = k2gp(ikx_.*Sk(:,:,2) - iky_.*Sk(:,:,1)); 
   h = k2gp(Sk(:,:,3));
   pv_res = vxMuy - h*f;
...
%    v_res = ikx_.*psi_res;
   ...
   Sk_new = Sk;
   Sk_new(:,:,1) = Sk_new(:,:,1) - u_res*alpha;
   Sk_new(:,:,2) = Sk_new(:,:,2) - v_res*alpha;
   Sk_new(:,:,3) = Sk_new(:,:,3) - psi_res*alpha;
   return
