function [U,KE,PE,time] = sw1rk3nu(Ui,Ro,Bu,nu,nt,savestep)

%  [U,KE,PE,time] = sw1rk3nu(Ui,Ro,Bu,nu,nt,savestep)
%
...
% Straight HVisc doesn't work for large enough nu.  Need implicit...

% Defaults
dttune= .01;
Sdef = 4;

global KMAX KMAXBIG K NX NXBIG RO BU S NU
...
global KMAX KMAXBIG K NXBIG RO BU S NU
...
wnl(:,2) = wgbig(:,1).*wgbig(:,2);    % u*(v_x+V_x)
...
rk(:,1) =  uk(:,2) -BU*i*K.*uk(:,3) - .5*RO*i*K.*wnlk(:,1)- NU*K.^(2*S).*uk(:,1); 
rk(:,2) = -uk(:,1)               -    RO*wnlk(:,2)- NU*K.^(2*S).*uk(:,2);
